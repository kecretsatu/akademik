...
<div class="container">
			...
			<div class="col-xs-12">
				...
				<div class="form-inline row" style="border:0px solid black">
					...
					<div class="form-group cari-prodi col-xs-6">
						...
						<button type="button" class="btn btn-success btn-sm" onclick="rencana_studi_load_data()" style="margin-left:20px">GO</button>					
					</div>
				</div>
				</form>
			</div>
	...
	<nav class="navbar navbar-default" style="padding:10px; background:white;">
		<div class="box-body">				
			...
			<a id="tambah-rencana-studi" class="btn btn-primary btn-sm" href="<?php echo $REQUEST_URI; ?>/tambah" style="display:none"><span class="glyphicon glyphicon-plus"></span> Tambah</a>
			<br/><br/>
...
<script>
	...
	$(document).ready(function(){
		<?php if($isLoad){echo 'rencana_studi_load_data()';} ?>
	});
	
	function rencana_studi_load_data(){
		window.history.replaceState('', '', '?'+setParamsURL('#rencana-studi-frm-f'));
		$('#tambah-rencana-studi').attr('href','<?php echo $REQUEST_URI; ?>/tambah?'+setParamsURL('#rencana-studi-frm-f'));
		$('#tambah-rencana-studi').show();
	...
	function rencana_studi_callback(data){
		...
		$.each(data, function(index, array){
			...
			tbody += '<a href="javascript:void(0)" onclick="dataremovecallback(\'#remove_rencana_studi_'+n+'\', rencana_studi_load_data)" type="button" class="btn btn-danger btn-xs" style="width:25; margin-right:2px;"><span class="glyphicon glyphicon-remove"></span> </a>';
			tbody += '<form id="remove_rencana_studi_'+n+'"><input type="hidden" name="name" value="<?php echo getTableAlias(1,'rencana_studi'); ?>"/>';
			tbody += '<input type="hidden" name="ctl_kode_rencana_studi" value="'+array['prime']+'" /></form>';
			tbody += '</td>';
			tbody += '</tr>';
		});
		
		$(table_name+' tbody').html(tbody);
		buildTableGroupHeader(table_name, 'Semester ', 1, '500px', '100px');
